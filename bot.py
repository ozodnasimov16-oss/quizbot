...
if __name__ == '__main__':
    ...
    try:
        ...
        print("=" * 60)
        # ==================== GRUPPA QUIZ BUYRUG'I ====================

@bot.message_handler(commands=['gquiz'])
@subscription_required  # agar kerak bo'lsa
def start_gquiz(message):
    ...
    # Avvalgi testni tekshirish
    existing_game = quiz_manager.get_active_game_by_chat(message.chat.id)
    if existing_game:
        game_id = existing_game['game_id']
        markup = types.InlineKeyboardMarkup()
        markup.add(
            types.InlineKeyboardButton("⏹️ Joriy testni to'xtatish", 
                      callback_data=f"stop_game_{game_id}")
        )
        return bot.reply_to(
            message,
            f"⚠️ <b>Bu guruhda allaqachon faol test bor!</b>\n\n"
            f"Tashkilotchi: {existing_game['creator_name']}\n"
            f"Savollar: {existing_game['current_question']}/{existing_game['total_questions']}\n"
            f"Ishtirokchilar: {len(existing_game['players'])} kishi\n\n"
            f"Avvalgini to'xtatib keyin yangisini boshlang.",
            parse_mode='HTML',
            reply_markup=markup
        )
    
    # Foydalanuvchining savollari borligini tekshirish
    if user_id not in user_data or not user_data[user_id].get('questions'):
        return bot.reply_to(
            message,
            "❌ <b>Sizda savollar yo'q!</b>\n\n"
            "Avval shaxsiy chatda /load buyrug'i bilan savollar yuklang.",
            parse_mode='HTML'
        )
    ...
    try:
        ...
        if message.from_user.id not in admin_ids and not is_admin(message.from_user.id):
            return bot.reply_to(
                message,
                "❌ <b>Faqat guruh adminlari test boshlashi mumkin!</b>",
                parse_mode='HTML'
            )
    except Exception as e:
    ...
    for num in options:
        if num <= total_questions:
            buttons.append(types.InlineKeyboardButton(
                f"{num} ta", 
                callback_data=f"gquiz_count_{num}_{user_id}"
            ))
    ...
    # Hammasi variantini qo'shish
    if total_questions > 0:
        markup.row(types.InlineKeyboardButton(
            f"Hammasi ({total_questions} ta)", 
            callback_data=f"gquiz_count_all_{user_id}"
        ))
    
    bot.reply_to(
        message,
        f"🎮 <b>Guruh Testi Boshlash</b>\n\n"
        f"👤 Tashkilotchi: {message.from_user.first_name}\n"
        f"📚 Sizda {total_questions} ta savol bor\n\n"
        f"<b>Nechta savol bo'lsin?</b>",
        parse_mode='HTML',
        reply_markup=markup
    )

@bot.callback_query_handler(func=lambda call: call.data.startswith('gquiz_count_'))
def handle_gquiz_count(call):
    """Guruh testi uchun savollar sonini tanlash"""
    try:
        ...
        # Faqat tashkilotchi o'zgartira oladi
        if user_id != creator_id:
            bot.answer_callback_query(call.id, "❌ Siz tashkilotchi emassiz!")
            return
        
        # Savollar sonini aniqlash
        if count == 'all':
            question_count = len(user_data[creator_id]['questions'])
        else:
        ...
        markup = types.InlineKeyboardMarkup(row_width=3)
        markup.add(
            types.InlineKeyboardButton("⏱️ 15s", callback_data=f"gquiz_time_{count}_15_{creator_id}"),
            types.InlineKeyboardButton("⏱️ 30s", callback_data=f"gquiz_time_{count}_30_{creator_id}"),
            types.InlineKeyboardButton("⏱️ 45s", callback_data=f"gquiz_time_{count}_45_{creator_id}")
        )
        markup.add(
            types.InlineKeyboardButton("⏱️ 60s", callback_data=f"gquiz_time_{count}_60_{creator_id}"),
            types.InlineKeyboardButton("⏱️ 90s", callback_data=f"gquiz_time_{count}_90_{creator_id}")
        )
        
        bot.edit_message_text(
            f"✅ <b>{question_count} ta savol tanlandi</b>\n\n"
            f"⏱️ <b>Har bir savol uchun qancha vaqt?</b>\n\n"
            f"Tanlang:",
            chat_id=chat_id,
            message_id=call.message.message_id,
            parse_mode='HTML',
...
@bot.callback_query_handler(func=lambda call: call.data.startswith('gquiz_time_'))
def handle_gquiz_time(call):
    """Guruh testi uchun vaqtni tanlash"""
    try:
        parts = call.data.split('_')
        count = parts[2]
        ...
        # Savollarni olish
        all_questions = user_data[creator_id]['questions']
        ...
        # O'yinni yaratish
        game_id = quiz_manager.create_game(
            chat_id=chat_id,
            questions=selected_questions,
            creator_id=creator_id,
            creator_name=call.from_user.first_name,
        ...
        markup.add(types.InlineKeyboardButton(
            "🎮 Testga qo'shilish", 
            callback_data=f"join_quiz_{game_id}"
        ))
        markup.add(types.InlineKeyboardButton(
            "📊 Joriy holat", 
            callback_data=f"quiz_status_{game_id}"
        ))
        
        bot.edit_message_text(
            f"🎉 <b>GURUH TESTI BOSHLANDI!</b>\n\n"
            f"👤 Tashkilotchi: {call.from_user.first_name}\n"
            f"📊 Savollar: {question_count} ta\n"
            f"⏱️ Vaqt: {time_limit} soniya\n"
            f"👥 Ishtirokchilar: 1 kishi\n\n"
            f"✅ <b>Testga qo'shilish uchun tugmani bosing!</b>\n\n"
            f"Test 30 soniyadan keyin boshlanadi...",
            chat_id=chat_id,
            message_id=call.message.message_id,
            parse_mode='HTML',
            reply_markup=markup
        ...
        # 30 soniyadan keyin testni boshlash
        threading.Timer(30, start_group_questions, args=[game_id]).start()
        
    except Exception as e:
        bot.answer_callback_query(call.id, f"❌ Xatolik: {str(e)}")

def start_group_questions(game_id):
    """Guruh savollarini boshlash"""
    game = quiz_manager.active_games.get(game_id)
    if not game:
        return
    
    chat_id = game['chat_id']
    
    # O'yinni boshlangan deb belgilash
    game['started'] = True
    quiz_manager.save_games()
    ...
    try:
        bot.edit_message_text(
            ...
            f"⏳ 5 soniya qoldi...\n"
            f"👥 Ishtirokchilar: {len(game['players'])} kishi",
            chat_id=chat_id,
    ...
    send_group_question(game_id)

def send_group_question(game_id):
    """Guruh uchun savol yuborish"""
    game = quiz_manager.active_games.get(game_id)
    ...
    try:
        msg = bot.send_poll(
            chat_id=game['chat_id'],
            question=f"❓ Savol {game['current_question'] + 1}/{game['total_questions']}\n\n{q['question']}",
            options=q['options'],
            type='quiz',
            correct_option_id=q['correct'],
            is_anonymous=False,
            open_period=game['time_limit'],
            explanation=f"⏱️ {game['time_limit']}s | 👥 {len(game['players'])} kishi"
        )
        
        # Savol ma'lumotlarini saqlash
        game['current_poll_id'] = msg.poll.id
        game['poll_message_id'] = msg.message_id
        game['question_start_time'] = time.time()
        
        # Keyingi savolni rejalashtirish
        threading.Timer(game['time_limit'] + 5, send_group_question, args=[game_id]).start()
        ...
        print(f"Savol yuborish xatosi: {e}")
        # Xatolik bo'lsa, keyingi savolga o'tish
        game['current_question'] += 1
        threading.Timer(2, send_group_question, args=[game_id]).start()

@bot.poll_answer_handler()
def handle_group_poll_answer(poll_answer):
    """Guruh testi javoblari"""
    try:
        user_id = poll_answer.user.id
        poll_id = poll_answer.poll_id
        
        # Qaysi o'yinda ekanligini topish
        for game_id, game in quiz_manager.active_games.items():
            if game.get('current_poll_id') == poll_id and game['is_active']:
                # Javob vaqti
                response_time = time.time() - game.get('question_start_time', time.time())
                
                # Savol indeksi (oldingi savol)
                question_idx = game['current_question'] - 1
                
                if 0 <= question_idx < len(game['questions']):
                    # To'g'ri javob
                    correct_answer = game['questions'][question_idx]['correct']
                    user_answer = poll_answer.option_ids[0] if poll_answer.option_ids else -1
                    ...
                    # Har 3 ta savolda reyting
                    if question_idx % 3 == 0:
                        show_intermediate_leaderboard(game_id)
                
                break
                
    except Exception as e:
        print(f"Javob qayta ishlash xatosi: {e}")

@bot.callback_query_handler(func=lambda call: call.data.startswith('join_quiz_'))
def handle_join_quiz(call):
    """Testga qo'shilish"""
    try:
        game_id = call.data.split('_')[2]
        
        success = quiz_manager.add_player(
            game_id=game_id,
            user_id=call.from_user.id,
        ...
        if success:
            bot.answer_callback_query(call.id, "✅ Testga qo'shildingiz!")
            
            # Xabar yuborish
            game = quiz_manager.active_games[game_id]
            if game.get('started', False):
                bot.send_message(
                    game['chat_id'],
                    f"👋 {call.from_user.first_name} testga qo'shildi!\n"
                    f"👥 Jami: {len(game['players'])} kishi"
                )
        else:
            bot.answer_callback_query(call.id, "⚠️ Siz allaqachon qo'shilgansiz!")
            
    except Exception as e:
        bot.answer_callback_query(call.id, f"❌ Xatolik: {str(e)}")

def show_intermediate_leaderboard(game_id):
    """Oraliq reyting"""
    try:
        game = quiz_manager.active_games.get(game_id)
        if not game:
        ...
        leaderboard = quiz_manager.get_leaderboard(game_id, 5)
        
        if leaderboard:
            text = f"🏆 <b>Oraliq Reyting</b>\n"
            text += f"Savol: {game['current_question']}/{game['total_questions']}\n\n"
            
            for player in leaderboard:
                text += f"{player['rank']}. {player['username']} - {player['score']} ball\n"
...
def end_group_quiz(game_id):
    """Guruh testini tugatish"""
    try:
        final_results = quiz_manager.end_game(game_id)
        
        if not final_results:
            return
        
        chat_id = final_results['chat_id']
        
        # Yakuniy xabar
        text = f"🎉 <b>GURUH TESTI YAKUNLANDI!</b>\n\n"
        text += f"📊 Umumiy natijalar:\n"
        text += f"👥 Ishtirokchilar: {final_results['stats']['total_players']} kishi\n"
        text += f"📝 Savollar: {final_results['total_questions']} ta\n\n"
        text += f"🏆 <b>TOP 3 G'OLIBLAR:</b>\n\n"
        
        for player in final_results['final_leaderboard'][:3]:
            text += f"{player['rank']}. {player['username']}\n"
            text += f"   ⭐ {player['score']} ball | ✅ {player['accuracy']:.1f}%\n"
        
        bot.send_message(chat_id, text, parse_mode='HTML')
        
        # Har bir ishtirokchiga shaxsiy natija
        for player_id, player_data in final_results['players'].items():
            try:
                # O'rinni topish
                rank = 0
                for i, p in enumerate(final_results['final_leaderboard'], 1):
                ...
                bot.send_message(
                    ...
                    f"🏆 O'rin: {rank}\n"
                    f"⭐ Ball: {player_data['score']:.1f}\n"
                    f"✅ To'g'ri: {player_data['correct_answers']}/{final_results['total_questions']}\n"
                    f"📈 Aniklik: {(player_data['correct_answers']/final_results['total_questions']*100):.1f}%\n\n"
                    f"🎮 Yangi test: /gquiz",
                    parse_mode='HTML'
                )
            except:
        ...
        print(f"Test tugatish xatosi: {e}")

@bot.message_handler(commands=['gstop'])
def stop_gquiz(message):
    """Guruh testini to'xtatish"""
    if message.chat.type not in ("group", "supergroup"):
        return bot.reply_to(message, "❌ Faqat guruhlar uchun!")
    ...
    try:
        ...
        admin_ids = [admin.user.id for admin in chat_admins]
        if message.from_user.id not in admin_ids and not is_admin(message.from_user.id):
            return bot.reply_to(message, "❌ Faqat adminlar to'xtata oladi!")
    except:
        pass
    
    # Faol testni topish
    existing_game = quiz_manager.get_active_game_by_chat(chat_id)
    if not existing_game:
        return bot.reply_to(message, "❌ Faol test topilmadi!")
    
    # Testni to'xtatish
    end_group_quiz(existing_game['game_id'])
    bot.reply_to(message, "✅ Test to'xtatildi!")

@bot.message_handler(commands=['gstatus'])
def gquiz_status(message):
    """Guruh testi holatini ko'rsatish"""
    if message.chat.type not in ("group", "supergroup"):
        return bot.reply_to(message, "❌ Faqat guruhlar uchun!")
    
    existing_game = quiz_manager.get_active_game_by_chat(message.chat.id)
    if not existing_game:
        return bot.reply_to(message, "❌ Faol test yo'q!")
    
    game = existing_game
    leaderboard = quiz_manager.get_leaderboard(game['game_id'], 5)
    
    text = f"📊 <b>Test Holati</b>\n\n"
    text += f"👤 Tashkilotchi: {game['creator_name']}\n"
    text += f"📝 Savollar: {game['current_question']}/{game['total_questions']}\n"
    text += f"⏱️ Vaqt: {game['time_limit']}s\n"
    text += f"👥 Ishtirokchilar: {len(game['players'])} kishi\n\n"
    
    if leaderboard:
        text += f"🏆 <b>Joriy Reyting:</b>\n"
        for player in leaderboard:
            text += f"{player['rank']}. {player['username']} - {player['score']} ball\n"
    
    bot.reply_to(message, text, parse_mode='HTML')

print("🤖 Gruppa Test Bot ishga tushdi!")
# ==================== BOTNI ISHGA TUSHIRISH ====================