...
def handle_all_callbacks(call):
    """Barcha callback query'larni markaziy qayta ishlash"""
    try:
        data = call.data
        ...
        # Guruh test callbacks
        elif data.startswith('group_'):
            handle_group_callback(call)
        
        # Test davom ettirish/to'xtatish
        elif data in ['continue_quiz', 'stop_quiz_now']:
            handle_continue_stop(call)
        
        # Admin callbacks
        elif data in ['admin_stats', 'admin_clean', 'admin_broadcast']:
...
def handle_quiz_mode(call):
    ...
    if call.data == 'mode_count':
        ...
        buttons.append(types.InlineKeyboardButton(f"Barchasi ({total_questions})", callback_data="qcount_all"))
...
def handle_question_count(call):
    ...
    if count == 'all':
        user_data[user_id]['quiz_count'] = total
        user_data[user_id]['selected_questions'] = user_data[user_id]['questions'][:]
    else:
        count_int = int(count)
        user_data[user_id]['quiz_count'] = count_int
        user_data[user_id]['selected_questions'] = user_data[user_id]['questions'][:count_int]
    ...
    markup.add(
        ...
        types.InlineKeyboardButton("♾️ Cheksiz", callback_data="timer_off")
    )
...
def handle_timer(call):
    ...
    markup.add(
        ...
        types.InlineKeyboardButton("📌 Asl tartibda", callback_data="shuffle_no")
    )
...
def handle_shuffle(call):
    ...
    markup.add(
        ...
        types.InlineKeyboardButton("📋 Tartibda", callback_data="order_sequential")
    )
...
def handle_order(call):
    ...
    bot.edit_message_text(
        ...
        f"⏱️ Vaqt: {timer_text}\n"
        f"🎲 Javoblar: {'Aralash' if user_data[user_id].get('shuffle_answers') else 'Asl tartib'}\n\n"
        f"⚠️ <b>DIQQAT:</b> Ketma-ket 2 ta savol javobsiz qolsa test pauzaga olinadi!\n\n"
        f"To'xtatish: /stop\n"
...
def handle_group_callback(call):
    ...
    if data == 'group_mode_count':
        ...
        buttons.append(types.InlineKeyboardButton(f"Barchasi ({total_questions})", callback_data="group_qcount_all"))
        ...
        total = len(group_quizzes[group_key]['questions'])
        
        if count == 'all':
            group_quizzes[group_key]['quiz_count'] = total
            group_quizzes[group_key]['selected_questions'] = group_quizzes[group_key]['questions'][:]
        else:
            count_int = int(count)
            group_quizzes[group_key]['quiz_count'] = count_int
            group_quizzes[group_key]['selected_questions'] = group_quizzes[group_key]['questions'][:count_int]
        ...
        markup.add(
            ...
            types.InlineKeyboardButton("📌 Asl tartibda", callback_data="group_shuffle_no")
        )
        ...
        markup.add(
            ...
            types.InlineKeyboardButton("📋 Tartibda", callback_data="group_order_sequential")
        )
        ...
        group_quizzes[group_key]['participants'] = {}
        ...
        bot.edit_message_text(
            ...
            f"📊 Savollar: {len(questions)}\n"
            f"⏱️ Vaqt: {timer_text}\n"
            f"🎲 Javoblar: {'Aralash' if group_quizzes[group_key].get('shuffle_answers') else 'Asl tartib'}\n\n"
            f"⚠️ <b>DIQQAT:</b> Ketma-ket 2 ta savol javobsiz qolsa test pauzaga olinadi!\n\n"
            f"Natijalarni ko'rish: /groupresults\n"
            f"To'xtatish: /stopgroup\n\n"
            f"Birinchi savol yuborilmoqda...",
            chat_id=chat_id,
            message_id=call.message.message_id,
            parse_mode='HTML'
...
def handle_continue_stop(call):
    """Davom ettirish/To'xtatish callbacks"""
    data = call.data
    user_id = str(call.from_user.id)
    chat_id = call.message.chat.id
    ...
    if data == 'continue_quiz':
        # Shaxsiy test
        if user_id in user_data and user_data[user_id].get('quiz_paused', False):
            user_data[user_id]['consecutive_skips'] = 0
            user_data[user_id]['quiz_paused'] = False
            ...
            time.sleep(1)
            send_poll_question(chat_id, user_id, is_group=False)
...
def start_group_quiz(message):
    ...
    if group_key in group_quizzes and group_quizzes[group_key].get('quiz_active', False):
        bot.send_message(
            chat_id,
            "⚠️ Guruhda allaqachon test boshlanagan!\n\n"
            "To'xtatish: /stopgroup\n"
            "Natijalarni ko'rish: /groupresults"
        )
        return
...
def poll_timeout_checker(chat_id, user_id, poll_id, is_group=False):
    ...
    timer = data_source[quiz_key].get('quiz_timer', 30)
    time.sleep(timer + 2)
    
    # Hali ham bir xil poll va quiz faolmi?
    current_poll = data_source[quiz_key].get('current_poll_id')
    quiz_active = data_source[quiz_key].get('quiz_active', False)
    
    if current_poll == poll_id and quiz_active:
        ...
        if data_source[quiz_key]['consecutive_skips'] >= 2:
            ...
            bot.send_message(
                ...
                f"⚠️ Yana bittasi o'tkazilsa test pauzaga olinadi.\n\n"
                f"Keyingi savol...",
                parse_mode='HTML'
            )
            time.sleep(2)
...
def handle_poll_answer(poll_answer):
    ...
    for group_key, group_data in group_quizzes.items():
        if group_data.get('current_poll_id') == poll_answer.poll_id:
            ...
            # Guruhda keyingi savolga o'tmaslik kerak
            # Poll_timeout_checker tomonidan avtomatik o'tkaziladi
...
def skip_question(message):
    ...
    if message.chat.type in ['group', 'supergroup']:
        ...
        if group_key not in group_quizzes or not group_quizzes[group_key].get('quiz_active', False):
            bot.send_message(chat_id, "❌ Hozirda faol test yo'q!")
            return
        
        # Consecutive skips counter'ini oshirish
        if 'consecutive_skips' not in group_quizzes[group_key]:
            group_quizzes[group_key]['consecutive_skips'] = 0
        
        group_quizzes[group_key]['consecutive_skips'] += 1
        group_quizzes[group_key]['skipped_questions'] = group_quizzes[group_key].get('skipped_questions', 0) + 1
        
        # Agar 2 ta ketma-ket savol o'tkazib yuborilsa
        if group_quizzes[group_key]['consecutive_skips'] >= 2:
            ask_continue_or_stop(chat_id, user_id, is_group=True)
        else:
            # Faqat 1 ta o'tkazilgan bo'lsa
            group_quizzes[group_key]['current_question'] += 1
            
            bot.send_message(
                chat_id,
                f"⏭️ Savol o'tkazib yuborildi.\n"
                f"⚠️ Diqqat: Ketma-ket {group_quizzes[group_key]['consecutive_skips']}/2 - yana bittasi o'tkazilsa test pauzaga olinadi.",
                parse_mode='HTML'
            )
            time.sleep(1)
            send_poll_question(chat_id, user_id, is_group=True)
...
def continue_quiz_command(message):
    ...
    if message.chat.type in ['group', 'supergroup']:
        ...
        if not group_quizzes[group_key].get('quiz_paused', False):
            bot.send_message(chat_id, "❌ Test pauzada emas!")
            return
        ...
        bot.send_message(chat_id, "✅ Test davom ettirilmoqda...")
        time.sleep(1)
        send_poll_question(chat_id, user_id, is_group=True)
